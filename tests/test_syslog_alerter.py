...
class SyslogAlerterTest(syslog_alerter.SyslogAlerter):
    def __init__(self, perms_mask, is_snap):
        ...
        super().__init__(perms_mask, null_logger, MagicMock(), .005, is_snap)
...
def test_send_alert_directory(monkeypatch):
    ...
    at.syslog_logger.warning.assert_called_with("Found permissions matching mask 002 on directory: /tmp/test")
...
def test_send_alert_mask(monkeypatch):
    ...
    at.syslog_logger.warning.assert_called_with("Found permissions matching mask 372 on directory: /tmp/test")
...
def test_vulnerability_mitigated_alert(monkeypatch):
    ...
    assert not at.syslog_logger.warning.called
    assert at.syslog_logger.info.called
    at.syslog_logger.info.assert_called_with("Found permissions " \
...
def test_snap_prefix_stripped(monkeypatch):
    ...
    at.add_event_to_alert_queue(syslog_alerter.SNAP_HOSTFS_PATH_PREFIX + "/tmp", syslog_alerter.SNAP_HOSTFS_PATH_PREFIX + "/tmp", "test")

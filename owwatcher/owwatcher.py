...
class OWWatcher():
    ...
    def __init__(self, logger, syslog_logger):
    ...
    def _watch_for_world_writable_files(self, dir):
        while True:
            try:
                self.logger.info("Setting up inotify watches on %s and its subdirectories" % dir)
                i = inotify.adapters.InotifyTree(dir, mask=OWWatcher.EVENT_MASK)
    ...
    def _process_event(self, event):
        ...
        if self._is_world_writable(path, filename):
            self.logger.info("Found world writable file/directory. Sending alert.")
            self._send_ow_alert(path, filename)

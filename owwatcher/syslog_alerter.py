...
class SyslogAlerter():
    def __init__(self, perms_mask, logger, syslog_logger, alert_queue_timeout_sec=2, is_snap=False):
    ...
    def run(self):
        ...
        alerter_thread = threading.Thread(target=self._send_alerts,
                args=(), daemon=True)
    ...
    def _send_alerts(self):
        while self.try_read_queue:
            try:
                (watch_dir, event_path, filename) = \
                    self.alert_queue.get(block=True, timeout=self.alert_queue_timeout_sec)
            except queue.Empty:
                continue
    ...
    def _send_syslog_alert(self, watch_dir, event_path, filename):
        ...
        (full_path, new_event_path) = self._strip_snap_prefix_from_event_path(event_path, filename)
